...
int channel_put(int cd, int data)
{
    ...
    {
        if (channel->state == NFREE)
        {
            sleep(&channel->valid_put, &channel->data_lock); // wakes up when valid_put changes and releases data_lock
        }
        else
        {
...
int channel_take(int cd, int *data)
{
    ...
    while (channel->valid_take == 0)
    {
        if (channel->state == NFREE)
        {
            sleep(&channel->valid_take, &channel->data_lock); // wakes up when valid_take changes and releases data_lock
        }
        else
        {
            release(&channel->data_lock);
